...
to build-owners
  ...
  ask available-rented [
    ...
    hatch-owners 1 [  ;; create an owner inside this house
      ...
      ;; manage ownership, mortgaget, repayment and rent of the landlord of the house
      ask owner-temp [
  ...
  if count-notavailable-owned > 0 [
    create-owners count-notavailable-owned [
      ...
      ;let rent income * Affordability / (ticksPerYear * 100 ) ;; create rent
      ;set my-rent rent
      set mortgage (list (income * Affordability / ( interestPerTick * ticksPerYear * 100 )) ) ;; create mortgage
      let deposit mortgage * ( 100 /  (MaxLoanToValue - 1) )  ;; create deposit
      set repayment ( list ((mortgage * interestPerTick) / (1 - ( 1 + interestPerTick ) ^ ( - MortgageDuration * TicksPerYear ))) )
    ]
  ]
...
to manage-market-participation [occupiers]
  ...
  let not-well-off-mortgage occupiers with [on-market? = false and is-house? my-house and myType = "mortgage" and (sum (repayment) * TicksPerYear) > (eviction-threshold-mortgage * income * Affordability / 100)]
  ...
  let not-well-off-mortgage-stay not-well-off-mortgage with [count turtle-set my-ownership > 1 and count ( (turtle-set my-ownership) with [for-sale? = true] ) = 0]
  ...
  let well-off-mortgage occupiers with [on-market? = false and is-house? my-house and myType = "mortgage" and (capital >= (sum (mortgage)) * ( (100 / MaxLoanToValue) - 1 ))]
  ...
  if any? well-off-mortgage [
    ask well-off-mortgage [
      if propensity > propensity-threshold [enter-market self "buy-to-let"]
    ]
  ]
...
to update-owners
  ...
  ask owners with [is-house? my-house and count turtle-set my-ownership > 1 and sum (mortgage) > 0] [
    foreach my-ownership [ house-temp ->
      ...
      set capital capital + ((Savings / 100) * (income + sum (income-rent)))
      ]
      ;; removed in 17.7
  ...
  ask owners with [ is-house? my-house and myType = "mortgage" and sum (mortgage) > 0 and count turtle-set my-ownership = 1] [  ;; ask all owners who do have one house and mortgage to pay
    set mortgage replace-item 0 mortgage ( (item 0 mortgage) - ((item 0 repayment) - (interestPerTick * item 0 mortgage)) )  ;; mortgage will be reduced due to repayment
...
"" ""
PENS
"Owners" 1.0 0 -13840069 true "" "plot (mean [income] of owners with [mytype = \"mortgage\"] + mean [sum income-rent] of owners with [mytype = \"mortgage\"]) / 10000"
"Renters" 1.0 0 -13791810 true "" "plot mean [income] of owners with [mytype = \"rent\"] / 10000"
"Investors" 1.0 0 -2674135 true "" "plot mean [income] of owners with [myType = \"mortgage\" and propensity > propensity-threshold] / 10000"
...
"all houses" 1.0 0 -7500403 true "" "plot mean ([sale-price] of houses with [myType = \"mortgage\" and sale-price > 0])"
...
"mean" 1.0 0 -7500403 true "" "plot mean [income] of owners"
...
"" ""
PENS
"Owners" 1.0 0 -13840069 true "" "plot mean [capital] of owners with [myType = \"mortgage\"] / 10000"
"Renters" 1.0 0 -7500403 true "" "plot mean [capital] of owners with [myType = \"rent\"] / 10000"
"Investors" 1.0 0 -2674135 true "" "plot mean [capital] of owners with [myType = \"mortgage\" and propensity > propensity-threshold] / 10000"
