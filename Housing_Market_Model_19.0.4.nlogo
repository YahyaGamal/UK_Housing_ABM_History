  ...
  if calculate-wealth? = True [
    ...
    while [i < length my-ownership] [
      ...
      ask my-ownership-temp [
        set my-realtor max-one-of local-realtors [ valuation myself ]               ;; set the realtor that gives the current house the highest valuation to be my-realtor
        set myType "mortgage"
        ask my-realtor [set sale-price-temp valuation my-ownership-temp]
        set myType myType-temp
      ]
      ;type "sale-price = " type sale-price-temp type " | mortgage = " type mortgage-temp type "\n"
...
to manage-surplus-seller [seller seller-house]
  ...
  if is-administrator? seller [
    ask seller [
      ;; find the index of the seller-house
      let index-temp position seller-house my-ownership
      ;type "index-temp = " type index-temp type " | my-ownership = " type my-ownership type " | mortgage = " type mortgage type " | children = " type children type "\n"
      ;; find the mortgage of the seller-house
      let mortgage-temp item index-temp mortgage
      ...
      ask turtle-set children [
        ...
        ;type "Child " type self type " inherited " type (surplus / n-children) type " from " type myself type "\n"
      ]
      set mortgage remove-item index-temp mortgage
    ]
  ]
...
"" ""
PENS
"Gini" 1.0 0 -16777216 true "" "plot gini-index [wealth] of owners"
...
5. Bugs
	- Addressed a bug at initialisation where mortgageduration and LTV were checked againist different random numbers leading to possibilities where only one considers the agent as a first-time buyer
...
4. Stamp duty
	...
	- Added control over stamp duty at three levels labelled as StamDuty-A?, StampDuty-B? and StampDuty-C? (the thresholds for each level is described in the interface)
5. Age
