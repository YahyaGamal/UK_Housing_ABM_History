...
to step
  ...
  set n-owners count owners  ;; take a count of total owners at the moment
  set owner-occupiers owners with [ is-house? my-house ] ; all owners who are living
  let independents owners with [ not is-house? my-house and age = (independentAge * ticksPerYear) ]
...
to manage-age
  ask owners [
    ...
    ; if age is the same as death-age
    if age = death-age [
      owners-die self
      stop
    ]
    ...
    while [i < length breed-age] [
      ...
      ;; if the child reached the age of 18, create the owner agent
      if age = (item i breed-age) + (IndependentAge * ticksPerYear) [
        hatch-owners 1 [
          ; address owner agent parameters
          set color black  ;; make owner red
          set size 0.7   ;; owner easy to see but not too big
          ;; ages
          set age (IndependentAge * ticksPerYear)
          assign-breed-death-age self
          ;; parent
          set parent myself
...
"first-time buyers" 1.0 0 -7500403 true "" "plot count owners with [first-time? = true]"
...
1086
592
go nYears | at mYear, interest = mInterest %
...
1086
652
go nYears | at pYear, MaxLoanToValue = pLTV %
...
1086
711
go nYears | at kYear, RentToRepayment = kRTR
...
1087
812
go nYears | at jYear, StampDuty? = jSD
...
1089
878
go nYears | at iYear, MortgageDurationFirstTime = iFT
...
"At Independent Age" 1.0 0 -13791810 true "" "plot nIndependentThisTick"
...
1092
947
go nYears | at cYear, MaxDensity = cMaxDensity
...
set MaxLoanToValueFirstTime pLTV-FT
  ...
  <experiment name="240210_RTR_120to140" repetitions="20" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go
if (ticks / TicksPerYear) = kYear [
set RentToRepayment kRTR
