...
owners-own[
  ...
  income              ; current income (from work per year?)
...
to manage-market-participation [occupiers]
  ...
  let not-well-off-mortgage occupiers with [on-market? = false and is-house? my-house and myType = "mortgage" and (repayment * TicksPerYear) > (eviction-threshold-mortgage * income * Affordability / 100)]
  ...
  ask occupiers [
    ...
    if [myType] of my-house = "rent" [
      ...
      if is-owner? landlord [
        ask landlord [set income-rent income-rent - [my-rent] of myself]
      ]
...
to new-houses
  ...
  type "Constructing n houses = " type count houses * HouseConstructionRate / 100 type "\n"
...
to value-hosues [houses-for-sale houses-for-rent]
  ...
  if any? houses-for-rent [                                                      ;; if these houses exist
    ask houses-for-rent with [ date-for-rent = ticks ] [                         ;; ask each of those houses which are just on sale from now on
      ...
      type "in valuation, rent-price = " type rent-price type "\n"
    ]                                                                                  ;; update the average-price of each realtor
    ask realtors [                                                                 ;; ask each realtor
...
to-report valuation [ property ]    ;; realtor procedure
  ...
  if pType = "mortgage" [
    ...
    ;type "Mort: " type (multiplier * new-price) type "\n"
  ...
  if pType = "rent" [
    ...
    ;type "Rent: " type (multiplier * new-rent) type "\n"
...
to manage-surplus-buyer [buyer]
  ...
  ask buyer [
    ...
    ifelse [sale-price] of new-house > capital [
      ...
      set mortgage mortgage + (min (list (income * Affordability / ( interestPerTick * ticksPerYear * 100 ))
...
to manage-surplus-landlord [landlord landlord-house]
  ...
  if is-owner? [rented-to] of landlord-house [
    ask landlord [
      set income-rent income-rent + rent
    ]
  ]
end
...
to demolish-houses
   ...
   if any? records [  ;; if there are records left
    ...
    ask houses-rent with [for-rent? and rent-price <  minimum-price-rent] [type rent-price type "\n"]

    type "all houises to demolish = " type count houses-to-demolish type "\n"
    type "demolish for minimum price = " type count houses-sell with [ (for-sale? and sale-price <  minimum-price-sold )] type "\n"
    type "demolish for minimum rent = " type count houses-rent with [ (for-rent? and rent-price <  minimum-price-rent )] type "\n"
    type "end of life sale = " type count houses-sell with [ticks > end-of-life] type "\n"
    type "end of life rent = " type count houses-rent with [ticks > end-of-life] type "\n\n"
...
to update-owners
  ...
  ;; manage rents
  ask owners with [is-house? my-house and count turtle-set my-ownership > 1 and mortgage > 0] [
    set mortgage mortgage - ( repayment - (interestPerTick * mortgage) );; mortgage will be reduced due to repayment
    let rented-houses my-ownership with [is-owner? rented-to]
    let total-rent 0
    ;; add up the rent of houses with tenants (this represents income rent per tick)
    if any? rented-houses [
      ask rented-houses [set total-rent total-rent + rent-price]
    ]
    ;; assign the income rent of the owner
    set income-rent total-rent
    ;; calculate capital while adding income rent to the base income every tick
    set capital capital + ((Savings / 100) * (income + income-rent))
    ;type self type " | my-ownership = " type my-ownership type " | rented = " type rented-houses type "\n"
    ;type "income-rent = " type income-rent type "\n"
  ]

  ;; myType condition was considered againist the value 1. Modified to "mortgage" to align with the new model.
  ask owners with [ is-house? my-house and myType = "mortgage" and mortgage > 0 and count turtle-set my-ownership = 1] [  ;; ask all owners who do have one house and mortgage to pay
    set mortgage mortgage - ( repayment - (interestPerTick * mortgage) );; mortgage will be reduced due to repayment
  ...
  ask owners with [ is-house? my-house and myType = "rent" ] [  ;; ask all owners who do have houses and mortgage to pay
    set capital capital + ((Savings / 100) * (income - my-rent))
    if capital < 0 [
      set capital 0
      ]
    ]
...
"" ""
PENS
"Owners" 1.0 0 -13840069 true "" "plot (mean [income] of owners with [mytype = \"mortgage\"] + mean [income-rent] of owners with [mytype = \"mortgage\"]) / 10000"
...
"" ""
PENS
"houses for rent" 1.0 0 -14439633 true "" "plot median [rent-price] of houses with [myType = \"rent\" and for-rent? = true]"
"all houses" 1.0 0 -7500403 true "" "plot median [rent-price] of houses with [myType = \"rent\"]"
...
1. addressed an error when a downshocked owner has not more my-ownership that are not for sale. This situation leads the owner to evict and enter the rent market.
