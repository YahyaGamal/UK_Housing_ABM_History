...
to update-owners
  ...
  ask owners with [is-house? my-house and count turtle-set my-ownership > 1 and mortgage > 0] [
    ...
    ;type self type " | previous-capital = " type capital type "\n"
    set capital capital + ((Savings / 100) * (income + income-rent))
    ;type self type " | new-capital = " type capital type "\n"
  ]
...
1. Addressed an issue with low rent prices leading to demolishing rent houses (occurs when a house is bough on a buy-to-let market, put on the rent market and then not having an offer during the tick. This leads to not calling the valuation function, and rent values remains 0)
