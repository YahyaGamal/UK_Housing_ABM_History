...
owners-own[
  ...
  income              ; current income (from work)
  income-rent         ; income from rent
...
to reset
   ; baseline
    ...
    set Inflation 0   ;; inflation rate
...
;; draw the circumference of a circle at the given radius
  hatch 1 [  ;; based on current turtle, let's create/hatch a new turtle which inherit its parent's properties
    set pen-size 2 set color one-of base-colors set heading -90 fd radius  ;; set up pen size, color and radius for drawing a circle
...
to put-on-market
  type "Putting " type self type " on the market | myType = " type myType type "\n"
...
to build-owners
  ...
  ask available-owned [  ;; define each home-owner's properties
    ...
    hatch-owners 1 [  ;; create an owner inside this house
      ...
      ask my-house [
        set sale-price [mortgage] of myself + [deposit] of myself
        set rent-price 0
      ]
  ...
  ask available-rented [  ;; define each home-owner's properties
    ...
    hatch-owners 1 [  ;; create an owner inside this house
      ...
      set my-ownership nobody
      let owner-temp one-of owners with [mytype = "mortgage"]
      let house-temp myself
      ask owner-temp [set my-ownership (turtle-set my-ownership house-temp)]
      ...
      set mortgage 0
      set repayment 0
      ask my-house [ set sale-price 0 set rent-price rent ] ;; create rent-price = rent
    ]
  ]

  ;; assign owners (i.e., landlords) to available rented houses
  ...
  if count-notavailable-owned > 0 [
    create-owners count-notavailable-owned [
      ...
      set color black  ;; make owner red
      set size 0.7   ;; owner easy to see but not too big
      set my-house nobody  ;; owner claims its house
      set mytype "mortgage"
      assign-income ;; create income and capital for owner
      ;let rent income * Affordability / (ticksPerYear * 100 ) ;; create rent
      ;set my-rent rent
...
to assign-income-rent
  ...
  ask turtle-set my-ownership with [is-owner? rented-to] [
    set total-rent total-rent + (rent-price * TicksPerYear)
...
to reset-empty-houses
  ...
  ask houses with [ not (is-owner? my-occupier) and myType = "mortgage"] [  ;; loop each empty house owned?
    let local-houses houses with [distance myself < Locality and sale-price > 0 and myType = "mortgage"]
...
to reset-houses-quality
  ...
  set medianPriceOfHousesForSale median [sale-price] of houses with [is-owner? my-owner and myType = "mortgage"]  ;; get median price for all houses owned
  set medianPriceOfHousesForRent median [rent-price] of houses with [is-owner? my-owner and myType = "rent"]  ;; get median price for all houses not owned
...
to step
  type "\n****************************** " type ticks type " ******************************\n"
  type "Count of owners = " type count owners type "\n"
  let n-owners count owners  ;; take a count of total owners at the moment

  calculate-globals ; update interest rate and income
  if count houses with [my-owner = nobody and myType = "rent"] > 0 [type "After calculate-globals\n"]
  ...
  if count houses with [my-owner = nobody and myType = "rent"] > 0 [type "After shock-management\n"]
  ...
  if count houses with [my-owner = nobody and myType = "rent"] > 0 [type "After owners-leave\n"]
  set n-owners count owners  ;; take a count of total owners at the moment
  set owner-occupiers owners with [ is-house? my-house ] ; all owners who are living

  ; owners entering naturally
  new-owners n-owners
  if count houses with [my-owner = nobody and myType = "rent"] > 0 [type "After new-owners\n"]
  set n-owners count owners  ;; take a count of total owners at the moment
  set owner-occupiers owners with [ is-house? my-house ] ; all owners who are living

  ; owners leaving due to discouragement
  manage-discouraged
  if count houses with [my-owner = nobody and myType = "rent"] > 0 [type "After manage-discouraged\n"]
  set n-owners count owners  ;; take a count of total owners at the moment
  set owner-occupiers owners with [ is-house? my-house ] ; all owners who are living
  ...
  if count houses with [my-owner = nobody and myType = "rent"] > 0 [type "After manage-market-participation\n"]

  ; introduce new houses
  new-houses
  if count houses with [my-owner = nobody and myType = "rent"] > 0 [type "After new-houses\n"]

  ; trading and moving into houses
  trade-houses
  if count houses with [my-owner = nobody and myType = "rent"] > 0 [type "After trade-houses\n"]

  ;remove extras (this removes all the offers, but still keeps the houses on the market)
  remove-outdates
  if count houses with [my-owner = nobody and myType = "rent"] > 0 [type "After remove-outdates\n"]

  ; demolish old houses
  demolish-houses
  if count houses with [my-owner = nobody and myType = "rent"] > 0 [type "After demolish-houses\n"]
  ; decay the prices of houses
  update-prices
  if count houses with [my-owner = nobody and myType = "rent"] > 0 [type "After update-prices\n"]
  ; update the homeless owners and discourage those who exceed the maxhomelesslimit
  update-owners
  if count houses with [my-owner = nobody and myType = "rent"] > 0 [type "After update-owners\n"]
...
to calculate-globals
  ...
  if Inflation > 0 [
    ask owners [
      set income income * (1 + Inflation / ( TicksPerYear * 100 )) ;; every tick, income stay the same or varied by inflation, income per year
    ]
  ]
end
  ...
  [
    ...
    if any? owner-occupiers-owned [
      ...
      ask owner-occupiers-owned with [ [for-sale?] of my-house = false ][
        ...
        ifelse  ratio <= Affordability / 200 [
          ...
          set nUpShockedSell nUpShockedSell + 1   ;; add 1 to `nUpShocked`, meaning one more owner selling house due to income rise (not true now after adding buy-to-let, owners does not have to sell)
        ]
        [
    ...
    if any? owner-occupiers-rented [
      ...
      ask owner-occupiers-rented with [ [for-rent?] of my-house = false][ ;; ask all home-owners whose house is not for sale (in setup, all home-owners don't sell houses)
        let ratio my-rent * TicksPerYear / income ;; put yearly-repayment / income  under `ratio`
        ifelse  ratio <= Affordability / 200 [  ;; if ratio < half of Affordability %, meaning yearly-rent is easy and owner is rich
          ;; enter the mortgage market to buy my own house
        ...
        [
          if ratio > Affordability / 50 [   ;; if ratio > 2 * Affordability % , meaning yearly-rent is way to heavy for owners to bear, owner is poor
...
to owners-leave [n-owners]
  ...
  ask n-of (ExitRate * n-owners / 200) owners-rent [
  ...
  type "\nRenters left, there is " type count houses with [for-rent? = true and my-owner = nobody] type " with for-rent? = true and my-owner = nobody\n\n"
  ask n-of (ExitRate * n-owners / 200) owners-mortgage [  ;; ask randomly select (ExitRate% of all owners) number of home-owners to do ...
    ...
    ask my-ownership-temp [
      ...
      if is-owner? my-occupier [
        type "owners-leave in ownership-temp\n"
        let my-occupier-temp my-occupier
        evict my-occupier-temp
        enter-market my-occupier-temp "rent"
      ]
      if count houses with [my-owner = nobody and myType = "rent"] > 0 [type "After evict in owners-leave\n"]
  ...
  type "\nOwners left, there is " type count houses with [for-rent? = true and my-owner = nobody] type " with for-rent? = true and my-owner = nobody\n\n"
end
  ...
  if maxHomelessPeriod  > 0 [ ; meaning if this value is set
    ask owners with [ not is-house? my-house ] [  ;; ask each owner without a house
      ...
      if homeless > maxHomelessPeriod [ ;; if homeless duration is beyond limit, this owner will move out of the city (agent die)
        ...
        type self type " is discouraged\n"
        type "discouraged my-ownership = " type my-ownership type " | my-house = " type my-house type "\n\n"
        die
      ]
    ]
  ]
...
to manage-market-participation [occupiers]
  ...
  let not-well-off-mortgage occupiers with [on-market? = false and is-house? my-house and myType = "mortgage" and (repayment * TicksPerYear) > (eviction-threshold * income * Affordability / 100)]
  ...
  let not-well-off-rent occupiers with [on-market? = false and is-house? my-house and myType = "rent" and (my-rent * TicksPerYear) > (eviction-threshold * income * Affordability / 100)]
  ...
  ; poorer owners with myType = mortgage and mare than one house (these will stay and will only sell one of their houses)
  let not-well-off-mortgage-stay not-well-off-mortgage with [count turtle-set my-ownership > 1]
  ...
  let well-off-mortgage occupiers with [on-market? = false and is-house? my-house and myType = "mortgage" and (capital >= mortgage * ( (100 / MaxLoanToValue) - 1 ))]
  let well-off-rent occupiers with [on-market? = false and is-house? my-house and myType = "rent" and capital >= (savings-to-rent-threshold * my-rent)]
  ...
  if any? well-off-mortgage [enter-market well-off-mortgage "buy-to-let"]
  if any? well-off-rent [enter-market well-off-rent "mortgage"]
...
to evict [occupiers]
  type "evicting " type occupiers type "\n"
  type "on-market-type = " type [on-market-type] of occupiers type " | my-house = " type [my-house] of occupiers type "| my-ownership = " type [my-ownership] of occupiers type " | made-offer-on = " type [made-offer-on] of occupiers type "\n"
  ask occupiers [
...
to force-sell [sellers]
  ask sellers [
    ...
    let my-ownership-temp my-ownership
    ask my-house [set my-ownership-temp other turtle-set my-ownership-temp]
    type "Force sell: my house = " type my-house type " | my-ownership = " type my-ownership type "| my-ownership-temp = " type my-ownership-temp type "\nmy-ownership-temp = "
    if any? my-ownership-temp [ask my-ownership-temp [type self type ", "] type "\n"]
    ;; choose one random house to sell
    let house-to-sell one-of my-ownership-temp with [for-sale? = false]
...
to-report valuation [ property ]    ;; realtor procedure
  ...
  if ptype = "rent"[
    ...
    ;;YG
    ;; Modified to renting price
    [ set new-price median [ renting-price ] of local-sales-rent ] ;; assign the median price of all record houses to new-price
    [ let local-houses houses with [ distance myself <= Locality and myType = "rent" ];; if no local-sales exist, take neighboring houses around the current realtor under `local-houses`
      ifelse any? local-houses  ;; if local-houses exist
        [set new-price median [rent-price] of local-houses  ;; set the median price of all local-houses to be new-price
      ]
      [set new-price average-rent ] ;; otherwise set average-price of the realtor to be new-price (is realtor's average-price updated every tick?)
    ]
  ]
  ...
  if pType = "rent" [
    if old-price < 500 [ report multiplier * new-price ]  ;; if current sale-price is too low, just accept multiplier * new-price  as valuation price
  ]
...
to make-offer-mortgage [ houses-for-sale ]
  ...
  if on-market-type = "mortgage" [
    ...
    if is-house? my-house [ if [for-sale?] of my-house = true [set deposit deposit + ([ sale-price ] of my-house - mortgage) ]]
    ...
    if upperbound < 0 and not is-house? my-house []
    ...
    let interesting-houses houses-for-sale with [
      ...
      self != current-house ]
    ;; assure buyhers do not make offers on their ownership
    ask current-ownership [set interesting-houses other interesting-houses]
...
to-report follow-chain [buyer-tenant]
  ...
  if [on-market-type] of buyer-tenant = "rent" [
    ...
    type "recursion in rent\n"
    report follow-chain [my-occupier] of house-made-offer-on
  ]
  type buyer-tenant type " reached end without reporting anything\n"
end
...
to move-house
 ...
 ;; then move the seller to their new house etc.
  ...
  ;; if seller is an owner, calc the profit made by selling the house
  if is-owner? seller and [ myType ] of new-house = "mortgage" [
    ...
    if profit > 0 [ask seller [ set capital capital + profit ]]
    ;; calc the duty payable with the new house sale-price
    ...
    ifelse [ sale-price ] of new-house > capital  [
      ...
      set mortgage min (list (income * Affordability / ( interestPerTick * ticksPerYear * 100 ))
        ([ sale-price ] of new-house * MaxLoanToValue / 100 ))
      ;; after paying rest with capital, the remaining still kept inside capital
    ...
    ;; if buyer's capital is not enough for downpayment, set the negative remaining to 0.
    if capital < 0 [ set capital 0 ]
  ]

  ;; Adjustments to ownerships and my-house are all made after this point, anything before this is only budget and profit adjustments
  ; if the new house is for rent
  if [ myType ] of new-house = "rent" [ ;; if seller is an owner, calc the rent and assign.
    ...
    ;; manage the parameters of the renter and the house
    set my-rent [ rent-price ] of new-house
    set my-ownership nobody
    set made-offer-on nobody
    ask new-house [set my-occupier myself set rented-to myself]
  ]
  if [myType] of new-house = "mortgage" [
    ...
    ;; manage the parameters of the buyer and the house
    set my-ownership (turtle-set my-ownership new-house)
    set made-offer-on nobody
    ; if I own more than one house, put the new house on the market for rent
    ifelse count turtle-set my-ownership > 0 [
      ask new-house [
        set myType "rent"
        set my-owner myself
        set my-occupier nobody
    ...
    ; if I own no houses, move me to the new house
    [
      show-turtle
      move-to new-house
      set homeless 0
...
to move-house-temp [buyer-tenant]
  ...
  if on-market-type = "mortgage" or on-market-type = "buy-to-let" [
    ...
    type "I am " type buyer type "\n"
    type "on-market-type = " type [on-market-type] of buyer type " | my-house = " type [my-house] of buyer type " | my-ownership = " type [my-ownership] of buyer type " | made-offer-on = " type [made-offer-on] of buyer type " | myType = " type [myType] of buyer type "\n"
    manage-surplus-buyer buyer
    manage-ownership-buyer buyer
    type "After managing ownership\n"
    type "on-market-type = " type [on-market-type] of buyer type " | my-house = " type [my-house] of buyer type " | my-ownership = " type [my-ownership] of buyer type " | made-offer-on = " type [made-offer-on] of buyer type " | myType = " type [myType] of buyer type "\n\n"
    if is-owner? seller [
      type "seller " type seller type "\n"
      type "on-market-type = " type [on-market-type] of seller type " | my-house = " type [my-house] of seller type " | my-ownership = " type [my-ownership] of seller type " | made-offer-on = " type [made-offer-on] of seller type " | myType = " type [myType] of seller type "\n"
  ...
  if on-market-type = "rent"[
    ...
    type "I am " type tenant type "\n"
    type "on-market-type = " type [on-market-type] of tenant type " | my-house = " type [my-house] of tenant type " | my-ownership = " type [my-ownership] of tenant type " | made-offer-on = " type [made-offer-on] of tenant type " | myType = " type [myType] of tenant type "\n"
    manage-surplus-landlord landlord new-house
    manage-surplus-tenant tenant
    manage-ownership-tenant tenant
    type "After managing ownership\n"
    type "on-market-type = " type [on-market-type] of tenant type " | my-house = " type [my-house] of tenant type " | my-ownership = " type [my-ownership] of tenant type "\n" type " | made-offer-on = " type [made-offer-on] of tenant type " | myType = " type [myType] of tenant type "\n\n"
  ]
...
to manage-ownership-buyer [buyer]
  ...
  ask buyer [
    ...
    type house-temp
    if is-house? house-temp [type ": my-owner = " type [my-owner] of house-temp type " | myType = " type [myType] of house-temp] type "\n"
    if on-market-type = "mortgage" [
      ;; record the sale transaction
      hatch-records 1 [
        hide-turtle
...
to manage-ownership-seller [seller seller-house]
  ask seller [
    ...
    ;set my-ownership turtle-set (remove seller-house (list my-ownership))
  ]
end
...
to manage-surplus-landlord [landlord landlord-house]
  let rent [rent-price] of landlord-house
  ;; add the rent to the yearly income of the landlord
  ask landlord [
    set income-rent income-rent + (rent * TicksPerYear)
    set income income + rent
  ]
end
   ...
   if any? records [  ;; if there are records left
    ...
    ask houses-to-demolish [
      ...
      if myType = "mortgage" and for-sale? = false [
        ...
        ask self [type "demolishing " type self type ": myType = mortgage | for-sale? = false | my-owner = " type my-owner type " | my-occupier = " type my-occupier type "\n"]
        let my-occupier-temp my-occupier
        evict my-occupier-temp
        enter-market my-occupier-temp "mortgage"
        ask my-occupier-temp [ type self type " was the occupier: on-market-type = " type on-market-type type " | my-house = " type my-house type " | my-ownership = " type my-ownership]
        die
      ]
      ...
      if myType = "mortgage" and (for-sale? = true or for-rent? = true) [
        ...
        if is-owner? my-owner and is-owner? my-occupier [
          ...
          ask self [type "demolishing " type self type ": myType = mortgage | for-sale? = true | my-owner = " type my-owner type " | my-occupier = " type my-occupier type "\n"]
        ...
        ; if the house has an owner but there is nobody occupying it (can happen if it was for rent and is now put on the mortgage market)
        if is-owner? my-owner and not is-owner? my-occupier [
          ask my-owner [
            let my-ownership-temp my-ownership
            ask myself [set my-ownership-temp other turtle-set my-ownership-temp]
            set my-ownership my-ownership-temp
            ;set my-ownership remove myself (list my-ownership)
          ]
        ]
        if count owners with [not is-house? my-house and count turtle-set my-ownership > 0] > 0 [type "After evict my-owner\n"]
        if count owners with [not is-house? my-house and count turtle-set my-ownership > 0] > 0 [type "After enter-market mortgage\n"]
      ...
      if myType = "rent" [
        ...
        if is-owner? my-occupier [
          ...
          ask self [type "demolishing " type self type ": myType = rent | my-owner = " type my-owner type " | my-occupier = " type my-occupier type "\n"]

          let my-occupier-temp my-occupier
          evict my-occupier-temp
          if count owners with [not is-house? my-house and count turtle-set my-ownership > 0] > 0 [type "After evict my-occupier\n"]
          enter-market my-occupier-temp "rent"
          if count owners with [not is-house? my-house and count turtle-set my-ownership > 0] > 0 [type "After enter-market rent\n"]
        ]
        ask my-owner [
          set capital capital + [sale-price] of myself
          let my-ownership-temp my-ownership
        ...
        if count owners with [not is-house? my-house and count turtle-set my-ownership > 0] > 0 [type "After setting ownership\n"]
        die
      ]
      die
    ]
...
;    ask houses-sell with [ (ticks > end-of-life) or  ;; ask all houses, if its life is over its life limit or
;
;      (for-sale? and sale-price <  minimum-price-sold )] ;; if the house is for sale and sale-price < minimum-price
;
;    [ demolish sale-price true ]  ;; let's demolish the house
;
;    ask houses-rent with [ (ticks > end-of-life) or  ;; ask all houses, if its life is over its life limit or
;
;      (for-sale? and sale-price <  minimum-price-rent )] ;; if the house is for sale and sale-price < minimum-price
;
;    [ demolish sale-price false]  ;; let's demolish the house
  ]
end

to demolish [sp f]

  if is-owner? my-occupier [ ;; under the context of the house, check the household is owner or not
    ask my-owner [ ;; if it is owner, set my-house nobody
      set my-house nobody
      if (f) [set capital capital + sp]
...
to update-owners
  ...
  ;;YG
  ;; myType condition was considered againist the value 1, rather than "mortgage"
  ask owners with [ is-house? my-house and myType = "mortgage" and mortgage > 0 ] [  ;; ask all owners who do have houses and mortgage to pay
    set mortgage mortgage - ( repayment - (interestPerTick * mortgage) );; mortgage will be reduced due to repayment
    set capital capital + ((Savings / 100) * income)
  ...
  ;;YG
  ;; myType condition was condered againist the value 0, rather than "rent"
  ask owners with [ is-house? my-house and myType = "rent" ] [  ;; ask all owners who do have houses and mortgage to pay
    set capital capital + ((Savings / 100) * income) - (my-rent / TicksPerYear)
    if capital < 0 [
      set capital 0
      ]
    ]
...
"" ""
PENS
"Owners" 1.0 0 -13840069 true "" "plot mean [income] of owners with [mytype = \"mortgage\"] / 10000"
"Renters" 1.0 0 -13791810 true "" "plot mean [income] of owners with [mytype = \"rent\"] / 10000"
...
"" ""
PENS
"nUpShockedSell" 1.0 0 -13840069 true "" "plot nUpShockedSell"
"nDownShockedSell" 1.0 0 -2674135 true "" "plot nDownShockedSell"
"nUpShockedRent" 1.0 0 -13345367 true "" "plot nUpShockedRent"
"nDownShockedRent" 1.0 0 -1184463 true "" "plot nDownShockedRent"
...
"" ""
PENS
"owned" 1.0 0 -13840069 true "" "plot count houses with [ is-owner? my-owner and myType = \"mortgage\"]"
"rent" 1.0 0 -13791810 true "" "plot count houses with [ is-owner? my-owner and myType = \"rent\"]"
"empty (owned)" 1.0 0 -2674135 true "" "plot count houses with [ not is-owner? my-owner and myType = \"mortgage\"]"
"population" 1.0 0 -1184463 true "" "plot count owners"
"houses" 1.0 0 -16777216 true "" "plot count houses"
"empty (rented)" 1.0 0 -612749 true "" "plot count houses with [ not is-owner? my-owner and myType = \"rent\"]"
...
"all houses" 1.0 0 -7500403 true "" "plot median ([sale-price] of houses with [myType = \"mortgage\" and sale-price > 0])"
...
"overall" 1.0 0 -7500403 true "" "plot mean [income] of owners"
...
"" ""
PENS
"default" 1.0 0 -13791810 true "" "plot medianPriceOfHousesForRent"
